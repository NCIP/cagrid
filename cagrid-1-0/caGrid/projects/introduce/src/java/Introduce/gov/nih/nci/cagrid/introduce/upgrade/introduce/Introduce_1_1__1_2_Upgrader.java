...
public class Introduce_1_1__1_2_Upgrader extends IntroduceUpgraderBase {
	...
	protected void upgrade() throws Exception {
	    ...
	    //need to make sure to save a copy of hte introduce.xml to a prev file so that the
        ...
        Utils.copyFile(new File(getServicePath() + File.separator
            + IntroduceConstants.INTRODUCE_XML_FILE), new File(getServicePath() + File.separator
            + IntroduceConstants.INTRODUCE_XML_FILE + ".prev"));

        // make a copy of the properties to compate with next time
        Utils.copyFile(new File(getServicePath() + File.separator
            + IntroduceConstants.INTRODUCE_PROPERTIES_FILE), new File(getServicePath() + File.separator
		...
		getStatus().addDescriptionLine(
				"replaced build.xml and build-deploy.xml with new version");


		StringBuffer devsb = Utils.fileToStringBuffer(new File(getServicePath()
				+ File.separator + "dev-build-deploy.xml"));
		...
		FileWriter fw = new FileWriter(new File(getServicePath()
				+ File.separator + "dev-build-deploy.xml"));
		...
		for (int i = 0; i < getServiceInformation().getServices().getService().length; i++) {
			...
			File oldConstantsFile = new File(srcDir.getAbsolutePath()
			...
			File resourceContanstsF = new File(srcDir.getAbsolutePath()
					...
					+ File.separator + service.getName() + "Constants.java");
			...
			if (service.getResourceFrameworkOptions().getMain() != null) {
				...
				File oldServiceConfF = new File(srcDir.getAbsolutePath()
						+ File.separator + CommonTools.getPackageDir(service)
			...
			if (service.getResourceFrameworkOptions().getCustom() == null) {
				...
				File oldBaseResourceBaseF = new File(srcDir.getAbsolutePath()
						+ File.separator + CommonTools.getPackageDir(service)
						+ File.separator + "service" + File.separator
						+ "globus" + File.separator + "resource"
				...
				File oldResourceConfigurationF = new File(srcDir
						.getAbsolutePath()
						+ File.separator
			...
			while (resourceI.hasNext()) {
				...
				if (resource.getAttributeValue("name").equals("home")) {
					...
					if (type.endsWith(".BaseResourceHome")) {
						...
						sb.insert(sb.lastIndexOf(".") + 1, serviceName
								+ "ResourceHome");
						resource.setAttribute("type", sb.toString());
					}
				} else if (resource.getAttributeValue("name").equals(
						"configuration")) {
					String type = resource.getAttributeValue("type");
					StringBuffer sb = new StringBuffer(type);
					sb.delete(sb.lastIndexOf(".") + 1, sb.length());
		...
		FileWriter writer = new FileWriter(new File(getServicePath()
				+ File.separator + "jndi-config.xml"));
		writer.write(XMLUtilities.formatXML(XMLUtilities
				.documentToString(jndiDoc)));
		writer.close();
		getStatus()
				.addDescriptionLine(
		...
		File oldSoapJar = new File(getServicePath() + File.separator + "tools"
				+ File.separator + "lib" + File.separator
				+ "caGrid-1.1-Introduce-1.1-soapBindingFix.jar");
		if (oldSoapJar.exists() && oldSoapJar.canRead()) {
			oldSoapJar.delete();
		} else {
			throw new Exception("Cannot remove old soap fix jar: "
					+ oldSoapJar.delete());
		}
		...
		File serviceTasksJardir = new File("." + File.separator + "skeleton"
				+ File.separator + "tools" + File.separator + "lib");
		File[] serviceTasksCandidates = serviceTasksJardir
				.listFiles(serviceTasksFilter);
		if (serviceTasksCandidates.length == 1) {
			...
			if (serviceTasksJar.exists() && serviceTasksJar.canRead()) {
				Utils.copyFile(serviceTasksJar, new File(getServicePath()
						+ File.separator + "tools" + File.separator + "lib"
						+ File.separator + serviceTasksJar.getName()));
				getStatus()
						.addDescriptionLine(
								"added service tasks jar to enable patching the soap bindings that get generated for custom beans");
			} else {
				throw new Exception(
						"Cannot find or cannot read service tasks jar to copy into the service: "
								+ serviceTasksJar.getAbsolutePath());
			}
		} else {
			throw new Exception(
					"Cannot find service tasks jar to copy into the service");
		}
	...
	private final class OldJarsFilter implements FileFilter {
		...
		public boolean accept(File name) {
			String filename = name.getName();
			boolean core = filename.startsWith("caGrid-1.1-core")
					&& filename.endsWith(".jar");
			boolean advertisement = filename
					.startsWith("caGrid-1.1-advertisement")
					&& filename.endsWith(".jar");
			boolean introduce = filename.startsWith("caGrid-1.1-Introduce")
					&& filename.endsWith(".jar");
			boolean security = (filename
					.startsWith("caGrid-1.1-ServiceSecurityProvider") || filename
					.startsWith("caGrid-1.1-metadata-security"))
					&& filename.endsWith(".jar");
			boolean gridGrouper = (filename
					.startsWith("caGrid-1.1-gridgrouper"))
					&& filename.endsWith(".jar");
			if (gridGrouper) {
				hadGridGrouperJars = true;
			}
			boolean csm = (filename.startsWith("caGrid-1.1-authz-common"))
					&& filename.endsWith(".jar");
			if (csm) {
				hadCSMJars = true;
			}

			boolean otherSecurityJarsNotNeeded = (filename
					.startsWith("caGrid-1.1-gridca"))
					&& filename.endsWith(".jar");
			...
			return core || advertisement || introduce || security
					|| gridGrouper || csm || wsrf || mobius
	...
	private void upgradeJars() throws Exception {
		...
		for (int i = 0; i < serviceLibs.length; i++) {
			...
			if (deleted) {
				getStatus().addDescriptionLine(
						serviceLibs[i].getName() + " removed");
			} else {
				getStatus().addDescriptionLine(
						serviceLibs[i].getName() + " could not be removed");
			}
		}
		...
		File csmLibDir = new File("ext" + File.separator + "skeleton"
				+ File.separator + "csm" + File.separator + "lib");
		File grouperLibDir = new File("ext" + File.separator + "skeleton"
		...
		if (oldDkeletonLibFilter.hadGridGrouperJars) {
			...
			for (int i = 0; i < gridGrouperLibs.length; i++) {
				File out = new File(serviceLibDir.getAbsolutePath()
						+ File.separator + gridGrouperLibs[i].getName());
				try {
					Utils.copyFile(gridGrouperLibs[i], out);
					getStatus().addDescriptionLine(
							gridGrouperLibs[i].getName() + " added");
				} catch (IOException ex) {
					throw new Exception("Error copying library ("
							+ gridGrouperLibs[i] + ") to service: "
							+ ex.getMessage(), ex);
				}
			}
		}
		...
		if (oldDkeletonLibFilter.hadCSMJars) {
			...
			for (int i = 0; i < gridCSMLibs.length; i++) {
				File out = new File(serviceLibDir.getAbsolutePath()
						+ File.separator + gridCSMLibs[i].getName());
				try {
					Utils.copyFile(gridCSMLibs[i], out);
					getStatus().addDescriptionLine(
							gridCSMLibs[i].getName() + " added");
				} catch (IOException ex) {
					throw new Exception("Error copying library ("
							+ gridCSMLibs[i] + ") to service: "
							+ ex.getMessage(), ex);
				}
			}
		}
	...
	private void removeResourcePropertyProvidersFromConfig() throws Exception {
		for (int i = 0; i < getServiceInformation().getServices().getService().length; i++) {
			ServiceType service = getServiceInformation().getServices()
					.getService(i);
			ProviderTools.removeProviderFromServiceConfig(service,
					"GetRPProvider", getServiceInformation());
			ProviderTools.removeProviderFromServiceConfig(service,
					"GetMRPProvider", getServiceInformation());
			ProviderTools.removeProviderFromServiceConfig(service,
					"QueryRPProvider", getServiceInformation());
		}
	}
	...
	private void removeGetResourcePropertyMethods() throws Exception {
		...
		for (int i = 0; i < getServiceInformation().getServices().getService().length; i++) {
			ServiceType service = getServiceInformation().getServices()
					.getService(i);
			SyncSource syncsource = new SyncSource(getServiceInformation()
			...
			String serviceClient = getServiceInformation().getBaseDirectory()
					.getAbsolutePath()
					+ File.separator
					+ "src"
					+ File.separator
			...
			try {
				...
				throw new SynchronizationException(
						"Error parsing service interface:" + e.getMessage(), e);
			}
			...
			if (found) {
				JavaQName qname = JavaQNameImpl
						.getInstance("GetResourcePropertyResponse");
